...
const CourseManagementPage: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        ...
        if (confirmed) {
            try {
                // Calling deleteCourse (camelCase) as defined in the updated CourseService.ts
                        ...
                        <CardContent className="p-6">
                            {loading ? (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                    {courses.map((course) => (
                                        <Card key={course._id} className="relative group overflow-hidden rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 border border-gray-100 hover:border-blue-300">
                                            <CardContent className="p-5 pt-7 flex flex-col items-center text-center bg-white">
                                                {/* Course Icon/Thumbnail (can be dynamic based on subject or a generic icon) */}
                                                <div className="flex-shrink-0 h-24 w-24 rounded-full bg-gradient-to-br from-blue-100 to-cyan-100 flex items-center justify-center text-blue-800 font-extrabold text-4xl mb-4 border-4 border-white shadow-md">
                                                    {course.name.charAt(0).toUpperCase()}
                                                </div>
                                                <CardTitle className="text-xl font-bold text-gray-900 mb-1 line-clamp-1">{course.name}</CardTitle>
                                                {course.description && <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{course.description}</p>}
                                                {course.duration && <p className="text-xs text-muted-foreground mt-1">Duration: {course.duration}</p>}
                                                {course.fee !== undefined && <p className="text-xs text-muted-foreground mt-1">Fee: ${course.fee}</p>}
                                                {/* Displaying topics and batches if they exist and are relevant to show on card */}
                                                {course.topics && course.topics.length > 0 && (
                                                    <p className="text-xs text-muted-foreground mt-1">Topics: {course.topics.length}</p>
                                                )}
                                                {course.batches && course.batches.length > 0 && (
                                                    <p className="text-xs text-muted-foreground mt-1">Batches: {course.batches.length}</p>
                                                )}
                                            </CardContent>
