...
import { Batch, CreateBatchData, CourseRef, TeacherRef, StudentRef } from "@/types/Batch.Types";

// Mock API service functions (replace with your actual service calls)
const mockApiService = {
    getCourses: async (): Promise<CourseRef[]> => ([
        { _id: "course_1", name: "MERN Stack Development" },
        { _id: "course_2", name: "Data Science with Python" },
        { _id: "course_3", name: "Advanced UI/UX Design" },
    ]),
    getTeachers: async (): Promise<TeacherRef[]> => ([
        { _id: "teacher_1", name: "Mr. Ankit Sharma" },
        { _id: "teacher_2", name: "Ms. Priya Singh" },
        { _id: "teacher_3", name: "Dr. Rahul Gupta" },
    ]),
    getStudents: async (): Promise<StudentRef[]> => ([
        { _id: "student_1", name: "Amit Kumar" },
        { _id: "student_2", name: "Sunita Devi" },
        { _id: "student_3", name: "Rajesh Verma" },
        { _id: "student_4", name: "Pooja Mehta" },
        { _id: "student_5", name: "Vikas Singh" },
        { _id: "student_6", name: "Anjali Sharma" },
    ]),
...
const BatchForm: React.FC<BatchFormProps> = ({ batchToEdit }) => {
    ...
    const [formData, setFormData] = useState<CreateBatchData>(() => ({
        ...
        teacherId: batchToEdit?.teacherId || "",
        ...
        const fetchData = async () => {
            ...
            try {
                const [coursesData, teachersData, studentsData] = await Promise.all([
                    ...
                    mockApiService.getTeachers(),
                    mockApiService.getStudents(),
                ]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = isEditMode
                ? await mockApiService.updateBatch(batchToEdit._id, formData)
                : await mockApiService.createBatch(formData);
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <Input id="name" value={formData.name} onChange={handleInputChange} placeholder="e.g., MERN Evening - July 2024" />
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Define when the batch will run.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {WEEK_DAYS.map(day => (
                                    <Button key={day} type="button" variant={formData.schedule?.days.includes(day) ? "secondary" : "outline"} onClick={() => handleDayToggle(day)}>
                                        {day}
                                    </Button>
                                ))}
                            </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Assign students to this batch.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="p-3 border rounded-md min-h-[120px]">
                            <div className="flex flex-wrap gap-2 mb-4">
                                {formData.students?.length === 0 && <p className="text-sm text-muted-foreground">No students assigned yet.</p>}
                                {formData.students?.map(studentId => {
                                    const student = allStudents.find(s => s._id === studentId);
                                    return (
                                        <div key={studentId} className="flex items-center gap-2 bg-secondary text-secondary-foreground rounded-full px-3 py-1 text-sm animate-in fade-in-0 zoom-in-95">
                                            {student?.name || 'Unknown'}
                                            <button type="button" onClick={() => toggleStudent(studentId)} className="rounded-full hover:bg-muted p-0.5">
                                                <X className="h-3 w-3" />
                                            </button>
                                        </div>
                                    );
                            ...
                            <Select onValueChange={toggleStudent}>
                                ...
                                <SelectContent>
                                    ...
                                    {filteredStudents.map(student => <SelectItem key={student._id} value={student._id}>{student.name}</SelectItem>)}
                                    {filteredStudents.length === 0 && <p className="p-4 text-center text-sm text-muted-foreground">No matching students found.</p>}
                                </SelectContent>
                            </Select>
                        </div>
                    </CardContent>
