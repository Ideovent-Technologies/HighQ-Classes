...
const Batches = () => {
  ...
  if (loading) return <p className="p-4">Loading batches...</p>;
  if (error) return <p className="p-4 text-red-500">Error: {error}</p>;
  if (!data || !data.assignedBatches) return <p>No data available.</p>;
      ...
      {assignedBatches.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {assignedBatches.map((batch) => {
            ...
            return (
              <Card key={batch._id} className="shadow-md border border-gray-200">
