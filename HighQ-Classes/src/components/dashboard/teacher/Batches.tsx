...
const Batches = () => {
  ...
  if (loading)
    return <div className="p-10 text-center text-gray-500">Loading batches...</div>;
  if (error)
    return <div className="p-10 text-center text-red-500">Error loading data</div>;
  if (!data || !data.assignedBatches)
    return <div className="p-10 text-center text-gray-400">No batches assigned.</div>;
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
    {assignedBatches.map((batch) => {
      const students = Array.isArray(assignedStudents?.[batch.name])
          ...
          <CardContent className="p-6 space-y-6 relative z-10">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-100">
              ...
              {batch.startDate && batch.endDate ? (
                ...
                <span className="italic text-red-300">Date not set</span>
              )}
            </div>
            ...
            <div>
              ...
              <ul className="text-sm text-white space-y-1 max-h-32 overflow-y-auto pl-3 pr-1 scrollbar-thin scrollbar-thumb-blue-300/60 scrollbar-track-transparent">
                {(isExpanded ? students : students.slice(0, 3)).map((student) => (
              ...
              {students.length > 3 && (
                ...
                >
                  {isExpanded ? (
                    ...
                    <>
                      Show All ({students.length}) <ChevronDown className="w-4 h-4" />
                    </>
                  )}
                </button>
              )}
