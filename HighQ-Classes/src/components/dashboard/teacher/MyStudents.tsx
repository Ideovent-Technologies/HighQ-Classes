...
const MyStudents = () => {
  ...
  if (loading)
    return <p className="p-6 text-lg text-blue-600 animate-pulse">Loading students...</p>;
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-blue-100 via-white to-blue-200">
      <div className="max-w-6xl mx-auto space-y-8">
        <h2 className="text-4xl font-extrabold text-navy-800 text-center drop-shadow-md">👩‍🏫 My Students</h2>

        {assignedBatches.length === 0 ? (
          <p className="text-gray-600 text-center">You are not assigned to any batches yet.</p>
        ) : (
          assignedBatches.map((batch) => {
            ...
            return (
              ...
              >
                {/* Batch Info */}
                <div className="mb-2">
                  <h3 className="text-2xl font-bold text-blue-900 tracking-tight">{batch.name}</h3>
                  ...
                  <div className="mt-4">
                    ...
                    <ul className="space-y-2 text-sm text-gray-800">
                      {notices.map((notice) => (
                        ...
                        >
                          <span className="font-semibold text-yellow-800">{notice.title}:</span>{" "}
