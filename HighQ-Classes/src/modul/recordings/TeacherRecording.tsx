...
const TeacherRecording: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fffefb] via-[#fdf5f0] to-[#f5f7fd] flex items-center justify-center p-4">
      <div className="flex flex-col md:flex-row items-center gap-8 bg-white/30 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20">
        {/* Form Box */}
        <div className="w-full max-w-md text-[#1A2540]">
          <h2 className="text-3xl font-bold mb-6 text-center">
            📤 Upload New Recording
          </h2>
          <div className="grid gap-4">
            <div>
              <Label className="text-[#1A2540]">Title</Label>
              <Input name="title" value={formData.title} onChange={handleChange} className="bg-white/80" />
            </div>
            <div>
              <Label className="text-[#1A2540]">Subject</Label>
              <Input name="subject" value={formData.subject} onChange={handleChange} className="bg-white/80" />
            </div>
            <div>
              <Label className="text-[#1A2540]">Video File</Label>
              <Input
                name="file"
                type="file"
                accept="video/*"
            ...
            <div>
              <Label className="text-[#1A2540]">Date</Label>
              <Input type="date" name="date" value={formData.date} onChange={handleChange} className="bg-white/80" />
            </div>
            <Button onClick={handleUpload} className="bg-[#1A2540] hover:bg-[#0f1b33] text-white font-semibold mt-2">
              Upload
            </Button>
          </div>
        </div>
